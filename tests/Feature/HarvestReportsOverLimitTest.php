...
test('harvester search by number with hash works in reports', function () {
    $company = Company::factory()->create();
    $user = User::factory()->create(['company_id' => $company->id]);
    $product = Product::factory()->create(['company_id' => $company->id]);

    $harvester1 = Harvester::factory()->create(['company_id' => $company->id, 'name' => 'Jovan Živojinović']);
    $harvester2 = Harvester::factory()->create(['company_id' => $company->id, 'name' => 'Marko Marković']);
    ...
    $this->actingAs($user)->get('/harvest/reports?searchHarvesterName=%231')
        ->assertSuccessful();
});
